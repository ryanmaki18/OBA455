...
for(obs_num in 1:n)
{
  ...
  train.mlr = lm(price_actual~age + km + fuel_type_Diesel + fuel_type_Petrol + hp + 
                   met_color + automatic + cc + doors +  quarterly_tax + weight, train)
...
train.lr = glm(loan_status_actual~age + experience + income + family + ccavg +
                 education_graduate +  education_advanced + mortgage + securities_account + 
                cd_account + online + credit_card, train, family = "binomial")
...
validation = validation %>%
  mutate(loan_status_prediction = if_else(prob_prediction >= 0.5, 1, 0))
