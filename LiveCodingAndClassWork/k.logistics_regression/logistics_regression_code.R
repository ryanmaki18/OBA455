...
delays = delays %>%
  mutate(Origin = factor (Origin, levels = c("IAD", "BWI", "DCA")),
         Destiniation = factor(Destination, levels = c("LGA", "EWR", "JFK")), 
...
train.lr = glm(Flight_Status_actual ~ Day_Week + Dep_Hour + Origin + Destination + 
                 Carrier + Weather, train, family = "binomial")
