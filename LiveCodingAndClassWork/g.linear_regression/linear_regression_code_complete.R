...
toyota = toyota %>%
  mutate(fuel_type = factor(fuel_type, levels = c("Diesel", "Petrol", "CNG")))
...
toyota.mlr = lm(price ~ age + km + fuel_type_Diesel + fuel_type_Petrol + fuel_type_CNG +
                  hp + met_color + automatic + cc + doors + quarterly_tax + weight, toyota)
