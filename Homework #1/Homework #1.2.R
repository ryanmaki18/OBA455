...
#w
p = ggplot(temp5, aes(x = Income, y = Lot_Size)) +
  geom_point() 

print(p)

p=ggplot(temp5, aes(Income,Lot_Size)) +
  geom_point() +
  geom_smooth
print(p)
...
temp=ridmov%>%
  arrange(Ownership)%>%
  select(Income, Ownership)%>%
  mutate(Income = factor(Ownership))

p = ggplot(temp, aes(x = "Ownership of Riding Mower", y = "Income")) +
  geom_boxplot()
